...
outdir=paste0(base,"5b_IdenCelltype_",method,"PanHumanAzimuth/")
if (!file.exists(outdir)) dir.create(outdir, showWarnings=F)
outdir=paste0(base,"5b_IdenCelltype_",method,"PanHumanAzimuth/",filter,"/")
if (!file.exists(outdir)) dir.create(outdir, showWarnings=F)
...
sc2 <- CloudAzimuth(sc2)

md <- sc2@meta.data

head(md)

tt <-  table(md$customclassif,md$seurat_clusters)

tt



##sc2 <- SCTransform(sc2, vars.to.regress = "percent.mt", verbose = TRUE)
