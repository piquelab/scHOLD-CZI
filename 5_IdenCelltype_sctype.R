...
args <- c("/rs/rs_grp_schold/CZI/RNA/analysis/",0.15,"ALL","fastdemux",13)
#args <- c("/rs/rs_grp_schold/CZI/RNA/analysis/",0.15,"ALL","fastdemux",50)
...
outdirtemp=paste0(base,"5b_IdenCelltype_",method,"/")
if (!file.exists(outdirtemp)) dir.create(outdirtemp, showWarnings=F)
...
## RPR  AR, we removed  the preharmony stuff here, and we also removed loop for dimset/dims, we run them individually. 
...
## Dim 13 failed here,  so RPR is going to try to start from here. 
#opfn <- paste0(outdir,project,".perc_scores.harmony-sctype-",resset,".",dimset,".rds")
