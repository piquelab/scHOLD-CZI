...
treatments=c("CTRL","LPS","PHA","PHA-DEX") #not using "LPS-DEX"
treatmentsfirst=c("CTRL")
...
opfn <- paste0(base,method,"_pseudobulk_ctrl/",filter,"/",project,".",resset,".",dimset,".DESeq_countlists_wavefilt.bticfilt.RData")
load(opfn)
...
enricheroutFolder=paste0(resultsFolder,"enrichr/")
...
varsrun = subset(variables_df, variable %in% c(intsig$variable))

library(biomaRt)  
mart <- useDataset("hsapiens_gene_ensembl", useMart("ensembl"))
genes <- biomaRt::getBM(attributes = c("ensembl_gene_id","hgnc_symbol", "chromosome_name","transcript_biotype"), filters = c("transcript_biotype","chromosome_name"),values = list("protein_coding",1:22), mart = mart)
int <- merge(int,genes[,c(1:2)],by.x="phenotype_id",by.y="ensembl_gene_id",all.x=T)
intsig <- subset(int,interaction_padj<0.1 )

zoom="GxE"
lapply(varsrun$variable, function(var){
    cat("running",var,"\n")
    stats <- subset(int, variable==var)
    statssig <- subset(intsig, variable==var)
    vareGenes <- length(unique(statssig$phenotype_id))
    #ressub <- subset(res, treat==i & variable==var)
    names(stats)[22] <- "gene_symbol"
    names(statssig)[22] <- "gene_symbol"
    #first subset variables that have at least one cluster with 50DEGs
    if(vareGenes<100){ 
        cat("not enough DEGs","\n")
        } else{

em <- enricher(statssig$gene_symbol, TERM2GENE=msigdbr_t2g_BP, universe=stats$gene_symbol, pvalueCutoff =0.1)
  if(!is.null(em) & dim(em)[1]>0){
    cat("sig results for",var,"\n")
res <- dplyr::filter(em@result, p.adjust < 0.05)
res <- res[order(res$p.adjust),]
   fwrite(res, sep='\t', quote=F, row.names=F, file=paste0(enricheroutFolder,"enrichr.",var,".",run,".txt"))
}#sig results for enrichr
}#GxE eGenes
})

#trying per cluster 
lapply(varsrun$variable, function(var){
  lapply(unique(int$cluster),function(c) {
    cat("running",var,"\n")
    stats <- subset(int, variable==var & cluster==c)
    statssig <- subset(intsig, variable==var & cluster==c)
    vareGenes <- length(unique(statssig$phenotype_id))
    #ressub <- subset(res, treat==i & variable==var)
    names(stats)[22] <- "gene_symbol"
    names(statssig)[22] <- "gene_symbol"
...
res <- dplyr::filter(em@result, p.adjust < 0.05)
res <- res[order(res$p.adjust),]
   fwrite(res, sep='\t', quote=F, row.names=F, file=paste0(enricheroutFolder,"enrichr.",c,".",var,".",run,".txt"))
}#sig results for enrichr
}#GxE eGenes
})#cluster
})#var
...
###########################################################################
  ...
  if(!dim(res)[1]==0 ){
   ...
   fwrite(s_dge, sep='\t', quote=F, row.names=F, file=paste0(enricheroutFolder,"_gse_enrichr.txt"))
ss <- ldply(lapply(split(s_dge,s_dge$Description),function(i){
   ...
   fwrite(ss, sep='\t', quote=F, row.names=F, file=paste0(outFolder,"gse_enrichrtop.",var,".",run,".txt"))
  if(nn=="upregulated"){ 
      hc <- "indianred1"
      lc <- "darkred"
