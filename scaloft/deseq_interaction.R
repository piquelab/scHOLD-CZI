...
if(zingeRrun){
    run="income_PCs_sex_age_and_treats_adjusted_withWave_zingeR_treatint"
}else{
    run="income_PCs_sex_age_and_treats_adjusted_withWave_treatint"
}
...
if(zingeRrun){
    run="SES_PCs_sex_age_and_treats_generem_zingeR_treatint"
}else{
    run="SES_PCs_sex_age_and_treats_generem_treatint"
}
}
...
allvars=c(psychvarstorun[-c(46:66,72:86)])
secondrunvars=c(psychvarstorun[c(1:13,24:45)],"age")
updated_vars=c("ISEL_Mean")
}
#firstrunvars <- "psesl"
...
for (cluster in names(counts_ls)){
    ...
    contrastdf_base <- data.frame(control=c("CTRL","CTRL","PHA"),treatment=c("LPS","PHA","PHA.DEX"))
    } else if(job=="CZI"){
    ...
    contrastdf_base <- data.frame(control=c("RNA.CTRL"),treatment=c("RNA.LPS"))
    }
        lapply(secondrunvars,function(var){
        #var="pnsi" var="DSES_07"
            contrastdf <- transform(contrastdf_base, contrast1=paste0(var,".","treats",control),contrast2=paste0(var,".","treats",treatment))
            ...
            cluster_metadata_var <- cluster_metadata[,c("Sample_ID","Wave","genPC1","genPC2","genPC3","Sex","treats",var)]
            ...
            cluster_metadata_var <- cluster_metadata[,c("Sample_ID","PC1","PC2","sex_alph","age","treats",var)]
            } else if(job=="CZI"& var=="age"){
            cluster_metadata_var <- cluster_metadata[,c("Sample_ID","PC1","PC2","sex_alph","treats",var)]
            }
            ...
            #for (x in 1:nrow(contrastdf)){
                ...
                design <-  paste0("~ PC1 + PC2 + sex_alph + ", var, " + treats + treats:",var)
                } else if (job=="CZI" & !var=="age"){
                design <-  paste0("~ PC1 + PC2 + sex_alph + age + ", var, " + treats + treats:",var)
                } else if(job=="CZI" & var=="CVDRISK"){
                design <-  paste0("~ PC1 + PC2 + ",var, " + treats + treats:",var)
                }
