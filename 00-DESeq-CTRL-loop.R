...
setwd("/wsu/home/hd/hd37/hd3768/piquelab/SCAIP_2022/Ali/09-ALOFT-SCAIP-batch-removed/00-ALOFT-SCAIP-pseudobulk-CTRL")

# create a table to save all stats, need modification for each treatment!
table <- data.frame(matrix(ncol=18, nrow=1))
x <- c("symb", "variable", "CTRL_NK_numb", "CTRL_NK_uniq_ID", "CTRL_NK_gene_numb", "CTRL_NK_DEGs", "CTRL_T_numb", "CTRL_T_uniq_ID", "CTRL_T_gene_numb", "CTRL_T_DEGs",
        "CTRL_B_numb", "CTRL_B_uniq_ID", "CTRL_B_gene_numb", "CTRL_B_DEGs", "CTRL_M_numb", "CTRL_M_uniq_ID", "CTRL_M_gene_numb", "CTRL_M_DEGs")
colnames(table) <- x
   
fname=paste("stats_all_cell_types", "_numbers_genes_degs", "_CTRL", ".txt", sep="")
...
sink(paste0(platePrefix,"_results_CTRL_treatment.txt"))
...
for(i in 1:length(variables)){
    ...
    cvNK <- cv[cv$MCls == "NKcell" & cv$treats == "CTRL",]
    cvT <- cv[cv$MCls == "Tcell" & cv$treats == "CTRL",]
    cvB <- cv[cv$MCls == "Bcell" & cv$treats == "CTRL",]
    cvM <- cv[cv$MCls == "Monocyte" & cv$treats == "CTRL",]
    
    #add treatment to the subset
    #CTRL_cvNK <- cv[cv$MCls == "NKcell" & cv$treats == "CTRL-EtOH",]
    ...
    table$CTRL_NK_numb <- paste(nrow(cvNK))
    table$CTRL_NK_uniq_ID <- paste(length(unique(cvNK$sample.ID)))
    table$CTRL_NK_gene_numb <- paste(nrow(dataNK))
    table$CTRL_NK_DEGs <- paste(nrow(resNK_sig))

    table$CTRL_T_numb <- paste(nrow(cvT))
    table$CTRL_T_uniq_ID <- paste(length(unique(cvT$sample.ID)))
    table$CTRL_T_gene_numb <- paste(nrow(dataT))
    table$CTRL_T_DEGs <- paste(nrow(resT_sig))

    table$CTRL_B_numb <- paste(nrow(cvB))
    table$CTRL_B_uniq_ID <- paste(length(unique(cvB$sample.ID)))
    table$CTRL_B_gene_numb <- paste(nrow(dataB))
    table$CTRL_B_DEGs <- paste(nrow(resB_sig))

    table$CTRL_M_numb <- paste(nrow(cvM))
    table$CTRL_M_uniq_ID <- paste(length(unique(cvM$sample.ID)))
    table$CTRL_M_gene_numb <- paste(nrow(dataM))
    table$CTRL_M_DEGs <- paste(nrow(resM_sig))

    # save it as a csv file

    #fname=paste(statsDir, '/', platePrefix,"_",myvar,"_all_cell_types_stats", "_numbers_genes_degs", "_CTRL", ".txt", sep="")
    fname=paste("stats_all_cell_types", "_numbers_genes_degs", "_CTRL", ".txt", sep="")
